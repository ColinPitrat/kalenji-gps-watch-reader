...
namespace device
{
	...
	void Keymaze::init()
	{
		...
		// Step 1: Some control transfer, necessary to initialize the device ?
		{
			...
			_dataSource->control_transfer(0x21, 0x01, 0x0404, 0x60, data, 0x0);
			...
			_dataSource->control_transfer(0x21, 0x01, 0x0404, 0x61, data, 0x0);
			...
			_dataSource->control_transfer(0xA1, 0x01, 0x0081, 0x0, dataIn, 0x1);
			DEBUG_CMD(std::cout << "Keymaze::init() - vendor device 18" << std::endl);
			_dataSource->control_transfer(0x21, 0x01, 0x0000, 0x1, data, 0x0);
			DEBUG_CMD(std::cout << "Keymaze::init() - vendor device 19" << std::endl);
			_dataSource->control_transfer(0x21, 0x01, 0x0001, 0x0, data, 0x0);
			DEBUG_CMD(std::cout << "Keymaze::init() - vendor device 20" << std::endl);
			_dataSource->control_transfer(0x21, 0x01, 0x0002, 0x0044, data, 0x0);
			...
			_dataSource->control_transfer(0xA1, 0x01, 0x0080, 0x0, dataIn, 0x1);
			...
			_dataSource->control_transfer(0x21, 0x01, 0x0B0B, 0x2, data, 0x0);
			DEBUG_CMD(std::cout << "Keymaze::init() - vendor device 36" << std::endl);
			_dataSource->control_transfer(0x21, 0x01, 0x0909, 0x0, data, 0x0);
			DEBUG_CMD(std::cout << "Keymaze::init() - vendor device 37" << std::endl);
			_dataSource->control_transfer(0x21, 0x01, 0x0808, 0x0, data, 0x0);
			// Class interface
			...
			_dataSource->control_transfer(0x11, 0x22, 0x0000, 0x0, data, 0x0);
			// Vendor device again
			_dataSource->control_transfer(0x21, 0x01, 0x0505, 0x1311, data, 0x0);
			// Class interface
