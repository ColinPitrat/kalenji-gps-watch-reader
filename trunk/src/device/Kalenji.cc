...
namespace device
{
	...
	void Kalenji::getSessionsList(SessionsMap *oSessions)
	{
		...
		for(int i = 0; i < nbRecords; ++i)
		{
			...
			double duration = (line [8] + (line[9] << 8) + (line[10] << 16) + (line[11] << 24)) / 10;
	...
	void Kalenji::getSessionsDetails(SessionsMap *oSessions)
	{
		...
		do
		{
			...
			{ // Decoding and addition of the lap
				...
				Lap *lap = new Lap(firstPoint, lastPoint, duration, length, max_speed, avg_speed, calories, grams, descent, ascent);
				session->addLap(lap);
			}
