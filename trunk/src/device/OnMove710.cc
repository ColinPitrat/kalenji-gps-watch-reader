...
namespace device
{
	...
	void OnMove710::parseGHPFile(unsigned char* bytes, int length, Session* session) 
	{
		...
		for(int i=0; i<length; i=i+20) 
		{
			...
			double speed = ((double)bytesToInt2(chunk[10],chunk[11])) * 10. / 3600.;//bytesToInt(chunk.slice(10, 12)) *10/3600,//[dm/h]=>[m/s]
	...
	void OnMove710::parseGHLFile(unsigned char* bytes, int length, Session *session) 
	{
		...
		for(int i=0;i<length;i=i+48) 
		{
			...
			int accruedTime = bytesToInt4(chunk[0],chunk[1],chunk[2],chunk[3]) * 100;//bytesToInt(chunk.slice(0, 4)) * 100, // [s/10] => [ms]
			int totalTime = bytesToInt4(chunk[4],chunk[5],chunk[6],chunk[7]) * 1000;//bytesToInt(chunk.slice(4, 8)) * 1000, // [s] => [ms]
			int totalDistance = bytesToInt4(chunk[8],chunk[9],chunk[10],chunk[11]);//bytesToInt(chunk.slice(8, 12)),
			int maxSpeed = bytesToInt2(chunk[12],chunk[13]) * 10 / 3600;//bytesToInt(chunk.slice(12, 14)) * 10 / 3600, // [dm/h] => [m/s]
			int averageSpeed = bytesToInt2(chunk[14],chunk[15]);//bytesToInt(chunk.slice(14, 16))*10/3600,[dm/h]=>[m/s]
			...
			/*uint32_t firstPointId;
			  ...
			  uint32_t descent;
			  uint32_t ascent;*/
			Lap *l = new Lap(startPoint, endPoint, totalTime, totalDistance, maxSpeed, 0, maxHeartRate, averageHeartRate, 0, 0, 0, 0);
			session->addLap(l);
		}
	}
	...
	void OnMove710::parseGHTFile(const unsigned char* bytes,Session* session) 
	{
		...
		int totalTime = bytesToInt4(bytes[8],bytes[9],bytes[10],bytes[11]) * 100;//bytes.slice(8, 12)) * 100, // [s/10] => [s]
		...
		int guestAverageSpeed = bytesToInt2(bytes[48],bytes[49]) * 10 / 3600;//bytesToInt(bytes.slice(48, 50)) * 10 / 3600,//[dm/h]=>[m/s]
		int guestAveragePace = bytesToInt2(bytes[50],bytes[51]);//bytesToInt(bytes.slice(50, 52)),
		int maxSpeed = bytesToInt2(bytes[52],bytes[53]) * 10 / 3600;//bytesToInt(bytes.slice(52, 54)) * 10 / 3600, // [dm/h] => [m/s]
		int averageSpeed = bytesToInt2(bytes[54],bytes[55]) * 10 / 3600;//bytesToInt(bytes.slice(54, 56)) * 10 / 3600,//[dm/h]=>[m/s]
